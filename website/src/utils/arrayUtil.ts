 ...
 * @returns A new array that contains all elements from the given arrays without duplicates.
 */
 ...
 * @param arr - The array in which to search for the largest element.
 * @returns The largest element in the array or `undefined` if the array is empty.
 */
export function findMax<T>(arr: T[]): T | undefined {
    if (arr.length === 0) return undefined;
    return Math.max(...arr as unknown as number[]) as unknown as T;
}

/**
 * Finds the smallest element in an array.
 * @param arr - The array in which to search for the smallest element.
 * @returns The smallest element in the array or `undefined` if the array is empty.
 */
export function findMin<T>(arr: T[]): T | undefined {
    if (arr.length === 0) return undefined;
    return Math.min(...arr as unknown as number[]) as unknown as T;
}
 ...
 * @returns An object where the key is the value of the specified key and the value is an array of objects.
 */
