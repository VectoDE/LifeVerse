...
const UserInfoCommand: Command = {
    ...
    async execute(interaction: ChatInputCommandInteraction) {
        try {
            ...
            if (!member) {
                await interaction.reply({ content: "❌ Couldn't fetch member information.", ephemeral: true });
                return;
            }

            const joinedAt = member.joinedAt ? member.joinedAt.toLocaleString() : "Not available";
            const createdAt = user.createdAt.toLocaleString();
            const roles = member.roles.cache.filter(role => role.id !== interaction.guild?.id).map(role => role.name).join(", ") || "No roles";
            ...
            const embed = new EmbedBuilder()
                ...
                .addFields(
                    ...
                    { name: "🕰️ Joined Server", value: joinedAt, inline: false },
