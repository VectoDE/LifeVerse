...
const PingCommand: Command = {
    ...
    async execute(interaction: ChatInputCommandInteraction) {
        ...
        try {
            ...
            const botStatus = botLatency < 200 ? `🟢 Online (${botLatency}ms)` : botLatency < 500 ? `🟠 Maintenance (${botLatency}ms)` : "🔴 Offline";

            const start = performance.now();
            await interaction.guild.members.fetch();
            const end = performance.now();
            const serverLatency = Math.round(end - start);
            const serverStatus = serverLatency < 200 ? `🟢 Online (${serverLatency}ms)` : serverLatency < 500 ? `🟠 Maintenance (${serverLatency}ms)` : "🔴 Offline";
                ...
                services.map(async (service) => {
                    try {
                        ...
                        if (response && response.status === 200) {
                            return { name: service.name, status: `🟢 Online (${latency}ms)` };
                        } else if (response && response.status === 503) {
                            return { name: service.name, status: `🟠 Maintenance (${latency}ms)` };
                        } else {
            ...
            const embed = new EmbedBuilder()
                ...
                .addFields(
                    ...
                    { name: "🖥️ Server Latency", value: serverStatus, inline: true },
                    ...serviceStatuses.map((s) => ({ name: s.name, value: s.status, inline: true }))
                );

            const chunkedGameServers = ArrayUtil.chunkArray(gameServerStatuses, 5);
            chunkedGameServers.forEach((chunk, _index) => {
                const serverFields = chunk.map((s) => ({ name: s.name, value: s.status, inline: true }));
