...
async function getRequestStats(): Promise<{ successCount: number, errorCount: number, totalRequests: number }> {
...
const LifeVerseInfoCommand: Command = {
    ...
    async execute(interaction: ChatInputCommandInteraction) {
        try {
            ...
            if (!guild) {
                await interaction.reply({ content: "❌ Unable to retrieve guild information.", ephemeral: true });
                return;
            }
            ...
            const gameServerStatuses = await Promise.all(config.apiRequests.GAME_SERVERS_LIST.map(async (server) => {
                const serverStatus = await checkURLStatus("GET", server.url);
                return { name: server.name, status: serverStatus ? "✅ Operational" : "🛑 Offline" };
            }));
            ...
            const pages = [
                new EmbedBuilder()
                    ...
                    .addFields(
                        ...
                        { name: "📜 Rules", value: `[Rules Channel](https://discord.com/channels/${guild.id}/1341925164392124509)`, inline: true },
                        { name: "🎭 Get Roles", value: `[Get Roles](https://discord.com/channels/${guild.id}/1341925450531737690)`, inline: true },
                    )
                    .setTimestamp(),
                ...
                new EmbedBuilder()
                    ...
                    .addFields(
                        { name: "🌐 Website", value: websiteStatus ? "[Visit LifeVerse Website](https://www.lifeversegame.com)" : "Website is **Offline**", inline: false },
                        { name: "📰 Latest News", value: "Check out the [latest updates](https://www.lifeversegame.com/news) on the website!", inline: false },
                        { name: "🔑 Sign Up / Sign In", value: "To get started, create an account or sign in to your existing account! [Sign In / Sign Up here](https://www.lifeversegame.com/sign-up)", inline: false },
                    )
                    .setTimestamp(),
                ...
                new EmbedBuilder()
                    ...
                    .setDescription("LifeVerse is a realistic 1:1 replica of the real world. Immerse yourself in a world that mirrors real life with stunning detail.")
                    .addFields(
                        { name: "🎮 Game Features", value: "Explore, socialize, and interact with others in a realistic setting.", inline: false },
                        { name: "🕹️ Platforms", value: "Available on **PC**, **Xbox One X**, and **PlayStation 5**.", inline: true },
                        { name: "📅 Release Date", value: "Coming soon in **2028**! [Download Here](https://download.lifeversegame.com)", inline: true },
                    )
                    .setTimestamp(),
                ...
                new EmbedBuilder()
                    ...
                    .setTitle("🌐 LifeVerse Website Info")
                    .addFields(
                        { name: "🌐 Website", value: websiteStatus ? "✅ Operational" : "🛑 Offline", inline: true },
                        { name: "📱 App", value: appStatus ? "✅ Operational" : "🛑 Offline", inline: true },
                        { name: "🛠️ API", value: apiStatus ? "🟢 Fully Operational" : "🛑 Offline", inline: true },
                        { name: "🔗 Database", value: databaseStatus ? "✅ Operational" : "🛑 Offline", inline: true },
                        { name: "🔐 Auth Service", value: authServiceStatus ? "✅ Operational" : "🛑 Offline", inline: true },
                ...
                new EmbedBuilder()
                    ...
                    .addFields(
                        { name: "✅ Successful Requests", value: `${stats.successCount}`, inline: true },
                        { name: "❌ Failed Requests", value: `${stats.errorCount}`, inline: true },
                        { name: "📊 Total Requests", value: `${stats.totalRequests}`, inline: true },
                        { name: "📈 Request Success Rate", value: progressBar, inline: false }
                    )
                    .setTimestamp(),
            ];
            ...
            try {
                message = await interaction.reply({ embeds: [embed], components: [row] });
            } catch (error) {
                LogService.error("Error replying with message:", error);
                await interaction.reply({ content: "❌ Failed to send initial message.", ephemeral: true });
                return;
            }
            ...
            await interaction.reply({ content: "❌ An error occurred while fetching LifeVerse information.", ephemeral: true });
        }
    },
};
